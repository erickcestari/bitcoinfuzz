...
void Driver::Bip39MnemonicToSeedTarget(std::span<const uint8_t> buffer) const {
  FuzzedDataProvider provider(buffer.data(), buffer.size());
  ...
  for (auto &module : modules) {
    std::optional<std::string> res{module.second->bip39_mnemonic_to_seed(mnemonic)};
    if (!res.has_value())
      continue;
    if (last_response.has_value()) {
      if (*res != *last_response) {
        std::cout << "Module: " << module.first << std::endl;
        std::cout << "Result: " << *res << std::endl;
        std::cout << "BIP39 mnemonic to seed failed for: " << mnemonic << std::endl;
        std::cout << "Module: " << last_module_name << std::endl;
        std::cout << "Result: " << *last_response << std::endl;
      }
      assert(*res == *last_response);
...
void Driver::Run(const uint8_t *data, const size_t size,
  ...
  } else if (target == "bip39_mnemonic_to_seed") {
    this->Bip39MnemonicToSeedTarget(buffer);
  } else {
