...
namespace bitcoinfuzz
{
    ...
    void Driver::ParseP2PMessageTarget(std::span<const uint8_t> buffer) const
    {
     ...
     void Driver::ParseGossipMessageTarget(std::span<const uint8_t> buffer) const
    {
        std::optional<std::string> last_response{std::nullopt};
        std::string last_module_name;
        ...
        for (auto &module : modules)
        {
            std::optional<std::string> res{module.second->parse_gossip_message(buffer)};
            if (!res.has_value()) continue;
            if (last_response.has_value()) {
                if (*res != *last_response) {
                    std::cout << "Gossip message parsing failed" << std::endl;
                    std::cout << "Module: " << module.first << std::endl;
                    std::cout << "Result: " << *res << std::endl;
                    std::cout << "Module: " << last_module_name << std::endl;
                    std::cout << "Result: " << *last_response << std::endl;
    ...
    void Driver::Run(const uint8_t *data, const size_t size, const std::string &target) const
    {
        ...
        } else if (target == "parse_gossip_message") {
            this->ParseGossipMessageTarget(buffer);
        } else {
