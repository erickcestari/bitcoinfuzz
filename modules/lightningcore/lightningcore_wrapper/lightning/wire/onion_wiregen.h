...
enum onion_wire {
        ...
        WIRE_MPP_TIMEOUT = 23,
        WIRE_INVALID_ONION_BLINDING = BADONION|PERM|24,
};
...
struct tlv_payload {
	...
	struct short_channel_id *short_channel_id;
        struct tlv_payload_payment_data *payment_data;
	u8 *encrypted_recipient_data;
	struct pubkey *current_path_key;
	u8 *payment_metadata;
	u64 *total_amount_msat;
};
struct tlv_encrypted_data_tlv {
	...
	u8 *path_id;
	struct pubkey *next_path_key_override;
...
struct blinded_path {
        struct sciddir_or_pubkey first_node_id;
        struct pubkey first_path_key;
        struct blinded_path_hop **path;
};
 ...
 */
struct tlv_payload *fromwire_tlv_payload(const tal_t *ctx,
 ...
 * detected.
 */
bool tlv_payload_is_valid(const struct tlv_payload *record,
			  size_t *err_index);

#define TLVS_ARRAY_SIZE_tlv_payload 8
...
enum tlv_payload_types {
	TLV_PAYLOAD_AMT_TO_FORWARD = 2,
	TLV_PAYLOAD_OUTGOING_CLTV_VALUE = 4,
	TLV_PAYLOAD_SHORT_CHANNEL_ID = 6,
	TLV_PAYLOAD_PAYMENT_DATA = 8,
	TLV_PAYLOAD_ENCRYPTED_RECIPIENT_DATA = 10,
	TLV_PAYLOAD_CURRENT_PATH_KEY = 12,
	TLV_PAYLOAD_PAYMENT_METADATA = 16,
	TLV_PAYLOAD_TOTAL_AMOUNT_MSAT = 18,
};

struct tlv_encrypted_data_tlv *tlv_encrypted_data_tlv_new(const tal_t *ctx);

/**
 * Deserialize a TLV stream for the tlv_encrypted_data_tlv namespace.
 *
 * This function will parse any TLV stream, as long as the type, length and
 * value fields are formatted correctly. Fields that are not known in the
 * current namespace are stored in the `fields` member. Validity can be
 * checked using tlv_encrypted_data_tlv_is_valid.
 */
struct tlv_encrypted_data_tlv *fromwire_tlv_encrypted_data_tlv(const tal_t *ctx,
						  const u8 **cursor, size_t *max);

/**
 * Serialize a TLV stream for the tlv_encrypted_data_tlv namespace.
 *
 * This function only considers known fields from the tlv_encrypted_data_tlv namespace,
 * and will ignore any fields that may be stored in the `fields` member. This
 * ensures that the resulting stream is valid according to
 * `tlv_encrypted_data_tlv_is_valid`.
 */
 ...
 * detected.
 */
bool tlv_encrypted_data_tlv_is_valid(const struct tlv_encrypted_data_tlv *record,
			  size_t *err_index);

struct tlv_onionmsg_tlv *tlv_onionmsg_tlv_new(const tal_t *ctx);

/**
 * Deserialize a TLV stream for the tlv_onionmsg_tlv namespace.
 *
 * This function will parse any TLV stream, as long as the type, length and
 * value fields are formatted correctly. Fields that are not known in the
 * current namespace are stored in the `fields` member. Validity can be
 * checked using tlv_onionmsg_tlv_is_valid.
 */
struct tlv_onionmsg_tlv *fromwire_tlv_onionmsg_tlv(const tal_t *ctx,
						  const u8 **cursor, size_t *max);

/**
 * Serialize a TLV stream for the tlv_onionmsg_tlv namespace.
 *
 * This function only considers known fields from the tlv_onionmsg_tlv namespace,
 * and will ignore any fields that may be stored in the `fields` member. This
 * ensures that the resulting stream is valid according to
 * `tlv_onionmsg_tlv_is_valid`.
 */
 ...
 * detected.
 */
bool tlv_onionmsg_tlv_is_valid(const struct tlv_onionmsg_tlv *record,
			  size_t *err_index);

/* SUBTYPE: BLINDED_PATH_HOP */
void towire_blinded_path_hop(u8 **p, const struct blinded_path_hop *blinded_path_hop);
struct blinded_path_hop *fromwire_blinded_path_hop(const tal_t *ctx, const u8 **cursor, size_t *plen);
