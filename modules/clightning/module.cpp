...
std::optional<std::string> clightning_des_invoice(const std::string &input) {
  ...
  struct bolt11 *invoice =
      bolt11_decode(tmpctx, input.c_str(), nullptr, nullptr, params, &fail);
  ...
  // with other implementations like LND. The multiplication and division by
  // 1000000000 may appear redundant, but it's intentionally mirroring the
  // nanosecond conversion logic used in LND to ensure the same overflow
  // characteristics.
  ...
  if (invoice->routes) {
    for (size_t i = 0; i < tal_count(invoice->routes); i++) {
      ...
      for (size_t j = 0; j < tal_count(route); j++) {
        ...
        result << ",PROPORTIONAL_MILLIONTHS="
               << route[j].fee_proportional_millionths;
        result << ")";
      }
      result << "]";
    }
...
std::string clightning_des_offer(const std::string_view input) {
  ...
  if (offer->offer_metadata) {
    result << hex_encode(offer->offer_metadata,
                         tal_bytelen(offer->offer_metadata));
  }
  ...
  if (offer->offer_features) {
    result << hex_encode(offer->offer_features,
                         tal_bytelen(offer->offer_features));
  }
  ...
  if (msg_type == WIRE_WARNING) {
    ...
    if (tlvs->remote_addr) {
      ...
      if (remote_addr->type == ADDR_TYPE_IPV6) {
        // Use hex representation to avoid differences between c-lightning and
        // rust-lightning formats for IPV6
        result << hex_encode(remote_addr->addr, remote_addr->addrlen) << ":"
               << remote_addr->port;
      } else {
    ...
    if (!fromwire_open_channel(
            tmpctx, msg, &chain_hash, &channel_id, &funding_sats, &push_msat,
            &dust_limit, &max_htlc_value_in_flight, &channel_reserve,
            &htlc_minimum, &feerate_per_kw, &to_self_delay, &max_accepted_htlcs,
    ...
    result << ";MAX_HTLC_IN_FLIGHT_MSAT="
    ...
    result << ";REVOCATION_BASEPOINT="
           << fmt_pubkey(tmpctx, &theirs.revocation);
    result << ";PAYMENT_BASEPOINT=" << fmt_pubkey(tmpctx, &theirs.payment);
    result << ";DELAYED_PAYMENT_BASEPOINT="
           << fmt_pubkey(tmpctx, &theirs.delayed_payment);
    result << ";HTLC_BASEPOINT=" << fmt_pubkey(tmpctx, &theirs.htlc);
    result << ";FIRST_PER_COMMITMENT_POINT="
           << fmt_pubkey(tmpctx, &first_per_commitment_point);
    result << ";CHANNEL_FLAGS=" << std::hex << std::setw(1) << std::setfill('0')
           << static_cast<int>(channel_flags) << std::dec;
    ...
    result << ";SIGNATURE="
           << fmt_secp256k1_ecdsa_signature(tmpctx, &signature);
    ...
    result << "MSG_TYPE=shutdown;CHANNEL_ID="
           << fmt_channel_id(tmpctx, &channel);
    ...
    result << ";PUBLIC_KEY="
           << fmt_secp256k1_pubkey(tmpctx, &onion->ephemeralkey.pubkey);
...
std::optional<std::string>
CLightning::deserialize_invoice(std::string str) const {
