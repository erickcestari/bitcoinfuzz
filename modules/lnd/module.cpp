...
namespace bitcoinfuzz
{
    namespace module
    {
        ...
        std::optional<std::string> Lnd::decode_onion(std::span<const uint8_t> buffer) const
        {
            auto result = LndDecodeOnion(reinterpret_cast<const char *>(buffer.data()), buffer.size());
            if (result == nullptr) {
                return std::nullopt;
            }
            std::string result_str(result);
